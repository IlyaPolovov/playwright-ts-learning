...
test('успешный логин по email', async ({ page }) => {
  const loginPage = new LoginPage(page)
  const { email, password } = generateLoginDataByEmail()
...
test('успешный логин по телефону', async ({ page }) => {
  const loginPage = new LoginPage(page)
  const { phone, password } = generateLoginDataByPhone()
...
//   });
  ...
  //   await expect(page).toHaveURL(/\/front_office\/whatsapp/)
  //   await expect(page.getByRole('link', { name: 'Выход' })).toBeVisible({ timeout: 30000 })
  // });
